...
if !exists("*Flake8()")
    function Flake8(select)
        ...
        if len(a:select) > 0
            let s:flake8_select=" --select=".a:select "tema: added for more flexible checking
        else
            let s:flake8_select=""
        endif
        " perform the grep itself
        let &grepformat="%f:%l:%c: %m\,%f:%l: %m"
        let &grepprg=s:flake8_cmd.s:flake8_builtins_opt.s:flake8_ignores.s:flake8_max_line_length.s:flake8_max_complexity.s:flake8_select
        silent! grep! %
